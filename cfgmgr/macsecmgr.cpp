...
constexpr std::uint64_t MKA_QUERY_INTERVAL_SECONDS = 5;
    ...
    std::string &error) const
{
    try
    {
        const auto output = wpa_cli_exec(
            session.sock,
            port_name,
            "",
            "macsec_mka_list");
    ...
    bool allowDesiredReplacement)
{
    ...
    session.next_query =
        ...
        std::chrono::seconds(MKA_QUERY_INTERVAL_SECONDS);
